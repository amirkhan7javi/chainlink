...
func (l *DRListener) handleOracleRequest(request *ocr2dr_oracle.OCR2DROracleOracleRequest, lb log.Broadcast) {
	...
	if errResult != nil {
		...
		if err2 := l.pluginORM.SetError(dbRequestID, run.ID, ERR_NODE_EXCEPTION, errResult.Error(), time.Now()); err2 != nil {
	...
	if errErr != nil {
		...
		if err2 := l.pluginORM.SetError(dbRequestID, run.ID, ERR_NODE_EXCEPTION, errErr.Error(), time.Now()); err2 != nil {
			l.logger.Errorf("Call to SetError failed for request ID: %v, DBID: %v", request.RequestId, dbRequestID)
		}
		return
	}
	...
	if len(computationError) != 0 {
		if err2 := l.pluginORM.SetError(dbRequestID, run.ID, ERR_USER_EXCEPTION, string(computationError), time.Now()); err2 != nil {
			l.logger.Errorf("Call to SetError failed for request ID: %v, DBID: %v", request.RequestId, dbRequestID)
		}
	} else {
